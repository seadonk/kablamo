...
export const fibAllObject = (n: number): { [key: number]: number } => {
  const result: { [key: number]: number } = {0: 1, 1: 1};
  for (let i = 2; i <= n; i++) {
...
export const factorialAll = (n: number): { [key: number]: number } => {
  const result: { [key: number]: number } = {1: 1};
  for (let i = 2; i <= n; i++) {
...
export const primeFactorization = (n: number, result: { [key: number]: number } = {}): { [key: number]: number } => {
  if (isPrime(n)) {
...
export const twinPrimesBelow = (max: number) => {
  ...
  return p.filter((t, i) => (t - p[i - 1] === 2) || (p[i + 1] - t === 2))
    .reduce((result: number[][], v, i, array) => {
      if (!(i % 2)) {
...
const actions: { [key: string]: (...rest: any) => any } = {
  fibRecursive,
  fibAllArray,
  fibAllObject,
  test,
